...
function createTempFixture(prefix) {
  ...
  const contextDir = path.join(tempHome, ".hitmux-context-engine");
...
async function runConcurrentMain(workerCount = 2) {
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "hitmux-context-engine-sync-lock-"));
  const contextDir = path.join(tempHome, ".hitmux-context-engine");
  const codebasePath = path.join(tempHome, "repo");
  const logFile = path.join(tempHome, "sync.log");
  ...
  const sharedEnv = {
    ...
    HITMUX_CONTEXT_ENGINE_SYNC_LOCK_STALE_MS: "60000"
  };
...
async function runStaleLockMain() {
  const { tempHome, contextDir, codebasePath, logFile } = createTempFixture("hitmux-context-engine-stale-sync-lock-");
  ...
  const worker = spawnWorker("stale", {
    ...
    E2E_LOG_FILE: logFile,
    E2E_CODEBASE_PATH: codebasePath,
    HITMUX_CONTEXT_ENGINE_SYNC_LOCK_STALE_MS: "1000"
  });
...
async function runHeldLockMain() {
  const { tempHome, contextDir, codebasePath, logFile } = createTempFixture("hitmux-context-engine-held-sync-lock-");
  const lockDir = path.join(contextDir, "mcp-sync.lock");
  fs.mkdirSync(lockDir, { recursive: true });
...
async function runEmptySnapshotMain() {
  const { tempHome, contextDir, codebasePath, logFile } = createTempFixture("hitmux-context-engine-empty-sync-lock-");
  fs.writeFileSync(path.join(contextDir, "mcp-codebase-snapshot.json"), JSON.stringify({
    formatVersion: "v2",
    codebases: {},
    lastUpdated: new Date().toISOString()
