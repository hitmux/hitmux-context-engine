...
async function withTempHome(run: (tempRoot: string) => Promise<void>): Promise<void> {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "hitmux-context-engine-mcp-snapshot-"));
    ...
    try {
        await fs.mkdir(path.join(homeDir, ".hitmux-context-engine"), { recursive: true });
        await run(tempRoot);
    } finally {
...
test("preserves request-level index options across snapshot state transitions", async () => {
    await withTempHome(async (tempRoot) => {
        ...
        const indexOptions = {
            ...
            requestIgnoreFiles: [".cursorignore"],
            requestMaxDepth: 2
        };
        ...
        assert.deepEqual(indexingInfo?.requestIgnoreFiles, [".cursorignore"]);
        assert.equal(indexingInfo?.requestMaxDepth, 2);
        ...
        assert.deepEqual(indexedInfo?.requestIgnoreFiles, [".cursorignore"]);
        assert.equal(indexedInfo?.requestMaxDepth, 2);
        ...
        assert.deepEqual(failedInfo?.requestIgnoreFiles, [".cursorignore"]);
        assert.equal(failedInfo?.requestMaxDepth, 2);
    });
});
...
test("explicit empty request options clear previous request-level index options", async () => {
    await withTempHome(async (tempRoot) => {
        ...
        assert.equal(info?.requestIgnoreFiles, undefined);
        assert.equal(info?.requestMaxDepth, undefined);
    });
});
...
test("preserves request-level index options when interrupted indexing is loaded as failed", async () => {
    await withTempHome(async (tempRoot) => {
        ...
        assert.deepEqual(info?.requestIgnoreFiles, [".cursorignore"]);
        assert.equal(info?.requestMaxDepth, 1);
    });
});
