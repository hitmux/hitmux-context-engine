...
export class SnapshotManager {
    ...
    constructor() {
        ...
        this.snapshotFilePath = path.join(os.homedir(), '.hitmux-context-engine', 'mcp-codebase-snapshot.json');
    }
 ...
 */
    ...
    private getIndexOptions(options?: CodebaseIndexOptions): CodebaseIndexOptions {
        ...
        if (options?.requestIgnoreFiles?.length) {
            indexOptions.requestIgnoreFiles = options.requestIgnoreFiles;
        }
        if (Number.isFinite(options?.requestMaxDepth) && options!.requestMaxDepth! >= 0) {
            indexOptions.requestMaxDepth = Math.floor(options!.requestMaxDepth!);
        }
        return indexOptions;
    }
