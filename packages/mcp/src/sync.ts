...
export class SyncManager {
    ...
    public async handleSyncIndex(): Promise<void> {
        ...
        if (!isAutoIndexingEnabled()) {
            console.log('[SYNC-DEBUG] Automatic indexing is disabled via config.autoIndexing=false.');
            return;
        }
    ...
    public startBackgroundSync(): void {
        ...
        if (!isBackgroundSyncEnabled()) {
            console.log('[SYNC-DEBUG] Background sync is disabled via config.backgroundSync=false.');
            return;
        }

        if (this.backgroundSyncEnabled) {
            console.log('[SYNC-DEBUG] Background sync polling is already active, skipping re-init');
            return;
        }
        ...
        console.log(`[SYNC-DEBUG] Background sync will repeat every ${syncIntervalMs}ms after each completed run`);

        console.log('[SYNC-DEBUG] Background sync setup complete. Timer ID:', this.backgroundSyncTimer);
    }

    private scheduleBackgroundSync(delayMs: number, reason: 'initial' | 'periodic'): void {
        this.backgroundSyncTimer = setTimeout(async () => {
            this.backgroundSyncTimer = null;
            const label = reason === 'initial'
            ...
            try {
                ...
                if (errorMessage.includes('Failed to query collection')) {
                    ...
                    console.error(`[SYNC-DEBUG] ${reason === 'initial' ? 'Initial' : 'Periodic'} sync failed with unexpected error:`, error);
                }
            } finally {
                if (this.backgroundSyncEnabled && this.backgroundSyncIntervalMs !== null) {
                    this.scheduleBackgroundSync(this.backgroundSyncIntervalMs, 'periodic');
                }
            }
        }, delayMs);
    }
    ...
    private setupTriggerWatcher(): void {
        if (!this.isTriggerWatcherEnabled()) {
            console.log('[SYNC-DEBUG] Trigger watcher disabled via config.triggerWatcher=false');
            return;
        }
