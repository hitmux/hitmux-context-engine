...
export class OllamaEmbedding extends Embedding {
    ...
    async embed(text: string): Promise<EmbeddingVector> {
        ...
        const embeddings = this.extractEmbeddings(response, 1, 'single embedding');
    ...
    async embedBatch(texts: string[]): Promise<EmbeddingVector[]> {
        ...
        const response = await this.client.embed(embedOptions);
        const embeddings = this.extractEmbeddings(response, texts.length, 'batch embedding');
    ...
    async detectDimension(testText: string = "test"): Promise<number> {
        ...
        try {
            ...
            const response = await this.client.embed(embedOptions);
            const embeddings = this.extractEmbeddings(response, 1, 'dimension detection');
            ...
            throw new Error(`Failed to detect Ollama embedding dimension for model "${this.config.model}": ${errorMessage}`);
        }
    }

    private extractEmbeddings(response: any, expectedCount: number, operation: string): number[][] {
        const embeddings = response?.embeddings;
        if (!Array.isArray(embeddings)) {
            throw new Error(`Ollama API returned invalid ${operation} response for model "${this.config.model}": missing embeddings array`);
        }

        if (embeddings.length !== expectedCount) {
            throw new Error(`Ollama API returned ${embeddings.length} embeddings for ${expectedCount} inputs using model "${this.config.model}"`);
        }

        embeddings.forEach((embedding, index) => {
            if (!Array.isArray(embedding) || embedding.length === 0 || !embedding.every((value) => typeof value === 'number')) {
                throw new Error(`Ollama API returned invalid embedding at index ${index} for model "${this.config.model}"`);
            }
        });
