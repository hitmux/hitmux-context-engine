...
export class ClusterManager {
    ...
    constructor(config?: ZillizConfig) {
        this.baseUrl = configManager.getString('zillizBaseUrl') || config?.baseUrl || 'https://api.cloud.zilliz.com';
        this.token = configManager.getString('milvusToken') || config?.token || '';

        if (!this.token) {
            throw new Error('Zilliz API token is required. Please provide it via config.milvusToken or config parameter.');
        }
    }
